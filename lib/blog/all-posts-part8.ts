...
export const BLOG_ARTICLES_PART8: BlogArticle[] = [
  {
    ...
    closing:
      "법인세 중간예납은 8월 31일까지 신고·납부해야 하며, 올해는 인상된 세율이 처음 적용되는 사업연도라는 점에서 예년과 다릅니다. 상반기 실적이 나빠졌다면 가결산 방식을 검토해보고, 계산 방식 선택이나 세액 규모 판단이 어렵다면 미루지 말고 확인하세요.",
  ...
  },
  {
    ...
    closing:
      "9월 재산세는 주택분 2기분과 토지분이 함께 몰리는 시기라 세액이 커 보일 수 있습니다. 20만 원 이하 주택은 이미 7월에 전액 납부됐을 가능성이 크고, 세액이 250만 원을 넘는다면 분할납부 신청 기한(9월 30일)을 놓치지 않는 것이 중요합니다. 보유세 부담이나 절세 방법이 궁금하다면",
    datePublished: checkedAt,
    dateModified: checkedAt,
    sources: [
  ...
  },
  {
    ...
    closing:
      "개인사업자 부가세 예정고지는 10월 26일까지 고지된 금액을 그대로 내는 것이 원칙이고, 30만 원 미만이면 고지 자체가 생략됩니다. 매출이 직전 대비 3분의 1 이상 줄었다면 예정신고로 전환해 부담을 줄일 수 있으니, 상황이 애매하다면 미루지 말고 확인해보세요. 부가세 신고나 매출 감소 시 절세 판단이 필요하다면",
    datePublished: checkedAt,
    dateModified: checkedAt,
    sources: [
